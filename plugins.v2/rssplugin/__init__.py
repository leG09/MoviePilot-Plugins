...
class RssPlugin(_PluginBase):
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "rssplugin_"
    # 加载顺序
    plugin_order = 19
    # 可使用的用户级别
    auth_level = 2
        ...
        for url in self._address.split("\n"):
            ...
            for result in results:
                try:
                    ...
                    # 生成媒体唯一标识，用于检查本次处理中是否已处理过
                    media_key = f"{mediainfo.tmdb_id or mediainfo.douban_id}_{mediainfo.type.value}"
                    if mediainfo.type == MediaType.TV and meta.begin_season:
                        media_key += f"_S{meta.begin_season:02d}"
                        if meta.episode_list:
                            media_key += f"_E{'-'.join(map(str, sorted(meta.episode_list)))}"
                    
                    # 检查本次处理中是否已处理过相同媒体
                    if media_key in processed_media:
                        logger.info(f'{mediainfo.title_year} {meta.season_episode} 本次处理中已处理过，跳过')
                        continue
                    # 种子
