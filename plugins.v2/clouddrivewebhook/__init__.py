...
class EmbyLibraryInfo:
    """Emby媒体库信息"""
    
    def __init__(self, Id: str, Name: str, CollectionType: str = None, Locations: list = None):
        self.Id = Id
        self.Name = Name
...
class CloudDriveWebhook(_PluginBase):
        ...
        return [
            {
                ...
                "description": "接收JSON格式的文件列表，只处理action为'create'的数据，通过gRPC直接刷新文件所在的上一级目录。支持批量处理多个文件路径。"
            },
            {
    ...
    def clouddrive_webhook(self, request_data: Dict[str, Any], apikey: Annotated[str, verify_apikey]) -> Dict[str, Any]:
        ...
        Args:
            request_data: 请求数据，包含data数组，每个元素有action和source_file字段；只处理action为"create"的数据；支持path_type参数："mapping"(默认)/"source"
        ...
        try:
            ...
            for i, item in enumerate(data):
                try:
                    ...
                    if action != "create":
                        logger.info(f"第 {i+1} 个文件action不是create，跳过处理: {action}")
                        skipped_files.append({"index": i, "reason": f"action_not_create: {action}"})
                        continue
